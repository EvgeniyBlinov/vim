...
class StatusWindow(Window):
    ...
    def on_create(self):
        ...
        output += "<F3>: step into\n"
        output += "<F4>: step out\n"
        output += "<F5>: start/run (to next breakpoint/end of script)\n"
        output += "<F6>: stop debugging\n"
        output += "<F7>: detach script from debugger\n"
        output += "<F9>: run to cursor\n"
        output += "<F10>: toggle line breakpoint\n"
        output += "<F11>: show context variables (e.g. after eval)\n"
        output += "<F12>: evaluate variable under cursor\n"
        output += ":Breakpoint <type> <args>: set a breakpoint of any type (see :help VdebugBreakpoints)\n"
        output += ":VdebugEval <code>: evaluate some code and display the result\n"
        output += "<Leader>e: evaluate the expression under visual highlight and display the result\n"
